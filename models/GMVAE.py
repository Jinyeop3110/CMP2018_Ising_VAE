...
class GMVAEFC(nn.Module):

    def __init__(self, N=16, hidden_dim=100, z_dim=2, k=2):
        super(GMVAEFC, self).__init__()
        ...
        self.decode2 = nn.Linear(self.hidden_dim, self.output_dim)
    ...
    def decode(self, z):
        ...
        return torch.tanh(self.decode2(h3))
    ...
    def Loss(self, x, px, z, zm, zv, zm_p, zv_p, qy, qy_logit):
        ...
        FLoss = torch.mean((ReconLoss + 1* KLD) * qy)+0.5*(torch.mean(NENT)-100*BNENT)+100*CLUST
    ...
    def Clustering(self,zm_p, zv_p):
        ...
        CLUST=CLUST+torch.sum(torch.relu(zv_p-0.5))
        return CLUST
