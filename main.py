...
from trainers.SpinTrainer_1120 import SpinTrainer_1120
import copyreg
...
def arg_parse():
    ...
    parser.add_argument('--model', type=str, default='spin',
                        choices=["spin", "VAEFC", "VAECONV"], required=True)
    # Unet params
...
if __name__ == "__main__":
    arg = arg_parse()
    arg.save_dir = "1124_basicIsing_VAE_TEST4"
    ...
    N=20
    z_dim=2
    f_path="/data1/0000Deployed_JY/IsingData/Ising_N20_a0"

    train_loader = SpinLoaderBasic(img_root=f_path, batch_size=arg.batch_size, shuffle=True)
    test_loader = SpinLoaderBasic(img_root=f_path, batch_size=1, shuffle=False)

    if arg.model == "VAEFC":
        net=VAEFC(N=N, hidden_dim=200, z_dim=z_dim)
    elif arg.model == "VAECONV":
        net=VAECONV(N=N, z_dim=z_dim)
    else:
    ...
    if(True):
        model = SpinTrainer_1120(arg, net, torch_device, recon_loss=recon_loss, val_loss=recon_loss, logger=logger)

        filename="epoch[0089]_losssum[165.800476].pth.tar"
